  ...
  const handleUser = async (e) => {
    ...
    try {
      ...
      if (msg?.toLowerCase().includes("no active account")) {
        setServerError(`We couldn't find an account associated with ${formData.email}.`);
      } else if (msg?.toLowerCase().includes("incorrect password")) {
        setServerError("The password you entered is incorrect. Please try again.");
      } else if (msg) {
        setServerError(msg);
      } else {
        setServerError("An unexpected error occurred while signing in. Please try again.");
      }
    }
  };
    ...
    <div className="min-h-screen bg-[#fffaf6] flex items-center justify-center py-10 px-4">
      <div className="w-full max-w-5xl grid gap-0 grid-cols-[1.05fr,0.95fr] md:flex rounded-[42px] overflow-hidden border border-[#f4decd] bg-white shadow-[0_30px_90px_rgba(201,142,102,0.22)]">
        <div className="bg-white px-6 sm:px-10 py-10 flex flex-col gap-8">
          ...
          {serverError && (
            <div className="flex gap-3 rounded-2xl border border-red-100 bg-red-50/80 px-4 py-3 text-sm text-red-700 shadow-sm">
