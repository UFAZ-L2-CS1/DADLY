...
const Navbar = () => {
  ...
  return (
    <section className='bg-[#fcf8f4] border-b-[0.5px] border-[#ecdacb] fixed w-full z-50'>
      <nav className='flex justify-between items-center max-w-[1400px]  mx-auto '>
        ...
        <ul className='flex font-semibold text-[#EB7A30] '>
        ...
        <div className='flex gap-2'>
          <button className='border-[#EB7A30] border-2 cursor-pointer text-[#EB7A30]  px-4 text-sm py-2 rounded-full flex items-center gap-2 hover:text-[#DF4B2D] hover:border-[#DF4B2D] hover:bg-[#f5e5d646] transition'>
            <BsFillStarFill />
            Rate Us
          </button>
         <Link to={`${user ? `/user/ur${currentUser?.id}/watchList` : '/token'}`} className='border-[#EB7A30] border-2 cursor-pointer text-[#EB7A30] px-4 text-sm py-2 rounded-full flex items-center gap-2 hover:text-[#DF4B2D] hover:border-[#DF4B2D] hover:bg-[#f5e5d646] transition'>
            ...
            Favourites
            {user ? <p className='px-2 bg-[#EB7A30] rounded-full text-[0.7rem]'>{currentUser?.watchList?.length}</p> : ''}
          </Link>
            ...
            <div className='flex  relative gap-2 text-white rounded-full px-4 py-1 hover:bg-white/10 cursor-pointer' onClick={() => handleUserData()}>
              ...
              {userDropdown && (
                <div className='bg-[#121212] border-1 rounded-sm border-[#ffffff1a] z-40 right-0 py-2 absolute text-white flex top-11 text-lg w-55 flex-col'>
                  <Link className='px-8 py-2 hover:bg-white/10' to={`/user/${currentUser?.id}/watchList`}>Your Watchlist</Link>
