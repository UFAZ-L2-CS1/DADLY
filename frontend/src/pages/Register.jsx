  ...
  const handleUser = async (e) => {
    ...
    try {
      ...
      if (msg?.toLowerCase().includes("already")) {
        setServerError(`This email (${user.email}) is already registered. Try signing in instead.`);
      } else if (msg) {
        setServerError(msg);
      } else {
        setServerError("Something went wrong while creating your account.");
      }
    }
  };
    ...
    <div className='min-h-screen bg-[#fffaf6] flex items-center justify-center py-10 px-4'>
      <div className='w-full max-w-6xl grid gap-0 grid-cols-[1.05fr,0.95fr]  md:flex rounded-[42px] overflow-hidden border border-[#f5decd] bg-white shadow-[0_30px_90px_rgba(201,142,102,0.22)]'>
        <div className='bg-white px-6 sm:px-10 py-10 flex flex-col gap-8'>
          ...
          {serverError && (
            <div className='border-2 rounded-2xl flex gap-3 items-start p-4 border-[#f5c095] bg-[#fff7ed] text-[#7f4a26]'>
              <GoAlertFill className='text-2xl mt-0.5 text-[#c4511d]' />
